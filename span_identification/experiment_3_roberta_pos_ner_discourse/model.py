# Span model with POS, NER, and discourse features.
import sys as _sys
...
class CFG:
    ...
    dropout: float = 0.3
    save_path: str = "best_span_roberta_pos_ner_discourse_2.pt"
...
def build_char_discourse_feature_map(text, nlp):
    """
    Returns:
        discourse_char_feats: np.ndarray of shape (len(text), 11)

    Features per character:
      0  sentence_relative_pos
      1  document_relative_pos
      2  paragraph_relative_pos
      3  is_in_first_sentence
      4  is_in_last_sentence
      5  is_in_quote
      6  is_title_like_sentence
      7  has_contrast_marker
      8  has_cause_marker
      9  has_emphasis_marker
      10 has_conclusion_marker
    """
    n_chars = len(text)
    num_feats = 11
    feats = np.zeros((n_chars, num_feats), dtype=np.float32)
...
class PTCSpanDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "pos_ids": ex["pos_ids"],
            "ner_ids": ex["ner_ids"],
            "discourse_feats": ex["discourse_feats"],
            "labels": ex["labels"],
...
class TransformerSpanTagger(nn.Module):
    ...
    def forward(
        ...
        if self.discourse_proj is not None and discourse_feats is not None:
            ...
            discourse_emb = self.discourse_proj(discourse_feats)
            if not torch.isfinite(discourse_emb).all():
                raise ValueError("Non-finite values detected in projected discourse features")
            feats.append(discourse_emb)
...
def train_one_epoch(model, loader, optimizer, scheduler, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Training")):
        ...
        if not torch.isfinite(discourse_feats).all():
            raise ValueError(f"Non-finite discourse_feats at step {step}")
...
def main():
    ...
    cfg = CFG(
        ...
        ner_dim=32,
        discourse_dim=16,
        save_path="best_span_roberta_pos_ner_discourse_2.pt"
    )
