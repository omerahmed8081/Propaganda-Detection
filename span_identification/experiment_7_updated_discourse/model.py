...
def _recall_contribution(gold_span, pred_spans):
    ...
    for pred_span in pred_spans:
        ...
        if credit > best:
            best = credit
    return best
def filter_spans(spans, max_len=200, min_len=3):
    return [
...
def predict_spans(model, loader, device):
    ...
    pred_spans_by_article = {
        aid: filter_spans(merge_overlapping_spans(spans))
...
def run_training(train_data, val_data,  cfg: CFG):
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if val_metrics["official_span_f1"] > best_val_official_f1:
            ...
            save_path = os.path.join(cfg.save_dir, f"{cfg.run_name}.pt")
            torch.save(best_state, save_path)
            meta = {
                "val_f1": best_val_official_f1,
                "config": cfg.__dict__
            }
...
def main():
    ...
    for exp in EXPERIMENTS:
        ...
        cfg = CFG(
            ...
            stride=exp["stride"],
            dropout=exp["dropout"],
            ce_loss_weight=exp["ce_loss_weight"],
            epochs=10,
            min_token_overlap_ratio=exp["min_token_overlap_ratio"],
            crf_loss_weight=0.5,
            lstm_hidden=384,
            lstm_layers=1,
            pos_dim=50,
            ...
            pos_scale=0.15,
            ner_scale=0.15,
            discourse_scale=0.15,
            class_weights=exp["class_weights"],
        )

        _ = run_training(train_data, val_data, cfg)

        meta_path = os.path.join("checkpoints_2", f"{cfg.run_name}_meta.json")
