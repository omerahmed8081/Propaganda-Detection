"""
Few-shot OpenRouter propaganda technique classification.
Use this to test outputs cheaply before running llm_technique.py with GPT.

Usage:
    python llm_openrouter.py --num 100
    python llm_openrouter.py --num 50 --model mistralai/mistral-7b-instruct
"""
...
# Default model — free tier on OpenRouter, good for testing
...
def call_openrouter(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://github.com/propaganda-detection",
        "X-Title": "Propaganda Technique Classifier",
    }
    payload = {
        "model": model,
        "messages": [
    ...
    # Free-tier models on OpenRouter return 429 when the model is at capacity.
    # Wait times grow exponentially: 30s, 60s, 90s, 120s, 150s, 180s.
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                wait = 30 * (attempt + 1)
                print(f"  Rate limited (model at capacity): {reason}")
    ...
    print("  Failed after all retries. The free model may be at capacity — try again later or use --model with a different free model.")
    return None
...
# MAIN PIPELINE
# =========================================================
def run(data_file: str, num_spans: int, output_dir: str, model: str):
    ...
    target = df.head(num_spans).reset_index(drop=True)
    print(f"Classifying {len(target)} spans with model: {model}\n")
    ...
    for idx, row in target.iterrows():
        ...
        raw         = call_openrouter(system_prompt, user_prompt, model)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Few-shot OpenRouter propaganda technique classification"
    )
    parser.add_argument(
        "--num",
        type=int,
        default=100,
        help="Number of spans to classify (default: 100)",
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        help=(
            ...
            "Other options: mistralai/mistral-7b-instruct:free, "
